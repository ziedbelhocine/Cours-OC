...
- `ydata-profiling` a été testé concrètement sur `df_merge` : confirmé utile en aval (diagnostic statistique) mais n'automatise ni le nettoyage ni la fusion (cf. livrable 1). OpenRefine, identifié ensuite comme alternative possible au script maison pour la consolidation, n'a lui pas été testé sur les données réelles — limite assumée (cf. livrable 1, section 4.2).
...
3. Tester OpenRefine sur un export réel (ex. `web.xlsx`) pour valider ou nuancer son intérêt comme outil de nettoyage accessible aux profils non techniques, en complément du script maison.
